...
Router.onBeforeAction(function() {
	Meteor.call('servlog', 'visited: ' + this.url);
	this.next();
});
...
Router.route('editor', {
	...
	fastRender: true,
	onBeforeAction: function() {
		document.title = 'ACAimmo - Options';
		if (Meteor.userId())
			this.next();
		else
			this.redirect('/admin');
	},
	waitOn: function() { return Meteor.subscribe('home_page'); },
	data: function() {
		if (this.ready())
			return HomePage.findOne();
	}
...
_.each(['agent','customer','estimation','propertie'], function(path) {
	Router.route(path, {
		...
		waitOn: function() {
			if (this.params.id && this.params.id == 'add')
				...
				if (path == 'customer') {
					return [
						Meteor.subscribe('customerMatch', this.params.id),
